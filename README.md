...
Each IP gets its own `deque` of request timestamps. The global 
traffic also has a deque. On every request:
...
- Formula: mean = sum(counts) / len(counts)
- stddev = sqrt(sum((x - mean)^2) / len(counts))
...
HNG-nginx-logs volume
      ↓
  Detector daemon (reads logs, detects anomalies)
      ↓
  ┌─────────────────────────────────┐
